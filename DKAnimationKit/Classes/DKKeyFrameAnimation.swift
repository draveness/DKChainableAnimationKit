...
public class DKKeyFrameAnimation: CAKeyframeAnimation {
    ...
    func createValueArray() {
        if let fromValue: AnyObject = self.fromValue, let toValue: AnyObject = self.toValue {
            if valueIsKindOf(NSNumber) {
                self.values = self.valueArrayForStartValue(CGFloat(fromValue.floatValue), endValue: CGFloat(toValue.floatValue)) as [AnyObject]
                ...
                if valueType.containsString("CGRect") {
                    ...
                    let path = self.createPathFromXYValues(self.valueArrayForStartValue(fromPoint.x, endValue: toPoint.x), yValues: self.valueArrayForStartValue(fromPoint.y, endValue: toPoint.y))
                    self.path = path
    ...
    private func valueArrayForStartValue(startValue: CGFloat, endValue: CGFloat) -> NSArray {

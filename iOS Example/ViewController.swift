...
class ViewController: UIViewController {
    ...
    let v: UIView = UIView(frame: CGRect(x: 10, y: 60, width: 50, height: 50))
    ...
    func animateView(sender: UIButton) {
        ...
        v.animation.anchorTopLeft.moveX(100).easeInOutExpo.thenAfter(0.5).moveWidth(50).bounce.makeBackground(green).easeIn.anchorTopLeft.thenAfter(0.5).rotate(95).easeBack.thenAfter(0.5).moveY(300).easeIn.makeOpacity(0.0).animateWithCompletion(0.4, {
            self.v.layer.transform = CATransform3DMakeRotation(0, 0, 0, 1)
            self.v.frame = CGRectMake(10, 60, 50, 50)
